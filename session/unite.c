...
pgm_t edit_pgm(pgm_t pgm1, pgm_t pgm2) {
  ...
  int x, y;
  pgm1.data = (unsigned char **)realloc(pgm1.data, output_pgm_width * output_pgm_height
              * sizeof(unsigned char));
  for (y = 0; y < output_pgm_height; y++) {
    pgm1.data[y] = (unsigned char *)realloc(pgm1.data[y], output_pgm_width * sizeof(unsigned char));
